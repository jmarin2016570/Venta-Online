...
function addFactura(req,res){
    ...
    Carrito.findOne({owner: userId},(err,cartFind)=>{
        if(err){
                    ...
                    if(err){
                        ...
                        User.findByIdAndUpdate(userId,{$push:{bills:billSaved._id}},{new:true},(err,userUpdated)=>{
                            if(err){
                                ...
                                Carrito.findOneAndRemove({owner: userId},(err,cartRemoved)=>{
                                    if(err){
...
function getProductosFactura(req,res){
    ...
    Factura.findById({_id:billId}).populate("products").exec((err,billFind)=>{
        if(err){
...
function getProductos(req,res){
    Factura.find({}).populate("products").exec((err,bills)=>{
        if(err){
